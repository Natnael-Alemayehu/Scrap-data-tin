...
def business_name_registered_number():
    ...
    print(business_name, registered_number)
    return business_name, registered_number
...
def person_position_name_and_image():
    div_tags = soup.find(
        "div", {"class": "flex flex-col items-center ng-star-inserted"}
    )
    # print(type(div_tags))
    if div_tags:
        image_link = div_tags.find(
            class_="sepia-0 w-28 h-28 bg-gray-300 rounded-md shadow mb-4 shrink-0 p-1"
        ).get("src")
    ...
    person_position = soup.find(
        "p", class_="text-md text-gray-700 uppercase font-bold tracking-wider mb-2"
    ).text
